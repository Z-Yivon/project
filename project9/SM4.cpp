...
const uint32_t SM4_SBOX_T0[256] = {
...
uint32_t load_u32_be(const uint8_t* b, uint32_t n) {
...
void store_u32_be(uint32_t v, uint8_t* b) {
    b[0] = (uint8_t)(v >> 24);
    b[1] = (uint8_t)(v >> 16);
    b[2] = (uint8_t)(v >> 8);
    b[3] = (uint8_t)(v);
...
uint32_t SM4_T_non_lin_sub(uint32_t X) {
    uint32_t t = 0;
...
uint32_t SM4_T_slow(uint32_t X) {
...
uint32_t SM4_key_sub(uint32_t X) {
    uint32_t t = SM4_T_non_lin_sub(X);
...
void sm4_ecb_encrypt(const uint8_t* in, uint8_t* out, const SM4_KEY* ks, size_t len) {
    const size_t block_size = 16;
    size_t num_blocks = len / block_size;
    uint8_t block_in[block_size];
    uint8_t block_out[block_size];

    for (size_t i = 0; i < num_blocks; i++) {
        for (size_t j = 0; j < block_size; j++)
            block_in[j] = *(in + i * block_size + j);
        sm4_encrypt(block_in, block_out, ks);
        for (size_t j = 0; j < block_size; j++)
            *(out + i * block_size + j) = *(block_out + j);
    }

    size_t last_block_size = len % block_size;
    if (last_block_size > 0) {
        for (size_t i = 0; i < last_block_size; i++)
            block_in[i] = (in + num_blocks * block_size)[i];

        uint8_t padding_value = 0;
        for (size_t j = last_block_size; j < block_size; j++)
            block_in[j] = 0;

        sm4_encrypt(block_in, block_out, ks);

        for (size_t k = 0; k < block_size; k++)
            *(out + num_blocks * block_size + k) = block_out[k];
    }
    else if (len > 0 && last_block_size == 0) {
        for (size_t j = 0; j < block_size; j++)
            block_in[j] = 0;
        sm4_encrypt(block_in, block_out, ks);
        for (size_t k = 0; k < block_size; k++)
...
int main() {
    ...
    sm4_ecb_encrypt(plaintext, ciphertext, &sm4_key, sizeof(plaintext));

    auto t2 = std::chrono::steady_clock::now();
    double dr_us = std::chrono::duration<double, std::micro>(t2 - t1).count();
    //cout << dr_us << "Î¢Ãë" << std::endl;
    printf("%fÎ¢Ãë\n", dr_us);
